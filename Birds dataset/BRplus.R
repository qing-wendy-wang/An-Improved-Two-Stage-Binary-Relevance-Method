...
####benchmark####
pred.label <- matrix(0,nrow=n,ncol=19)
...
for(j in 1:dim(label.birds)[2])
{
  ...
  for(k in 1:K)
  {
    ...
    mydata_BRplus_j <- as.data.frame( cbind(label.birds[,j],label.birds[,-j],x.matrix) )
    ...
    est.labels <- matrix(0,nrow=length(index.fold[[k]]),ncol=dim(label.birds)[2])
    est.labels <- as.data.frame(est.labels)
    colnames(est.labels) <- names(label.birds) 
    col.index= setdiff( c(1:dim(label.birds)[2]), j)
    
    #BR predictions for all labels that will be used for BRplus predictions
    for (v in 1:(dim(label.birds)[2]-1))
    {
      current = col.index[v]
      mydata_BR = as.data.frame( cbind(label.birds[,current],x.matrix)   )
    ...
    #It is possible that you may also need to change the var names in est.labels to the consistent with those in label.birds
